...
class ConvertSingleJsonAnswerCommand extends Command
{
    ...
    public function execute(Arguments $args, ConsoleIo $io)
    {
        ...
        foreach ($questions as $q) {
            ...
            foreach ($answers as $a) {
                ...
                $ad = '';
                if (!is_null($a->answer)) {
                    $ad = json_decode($a->answer, true); //answer decoded
                    if (json_last_error() != JSON_ERROR_NONE) {
                        $ad = $a->answer;
                        $a->answer = json_encode($ad);
                    ...
                    if ($type == 'single' && is_array($ad)) {
                        $io->out("\t\t Answer {$q->id} is multiple");
                    }
                    if ($type == 'multiple' && !is_array($ad)) {
                        $io->out("\t\t Answer {$q->id} is single");
                    }
                }
